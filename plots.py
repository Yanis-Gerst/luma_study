...
def plot_uncertainty_distributions(datasets, aggregations, aggregation_labels, output_filename):
    num_datasets = len(datasets)
    num_aggregations = len(aggregations)
    fig, axes = plt.subplots(num_datasets, num_aggregations, figsize=(
        num_aggregations * 3, num_datasets * 2))

    for row, dataset in enumerate(datasets):
        for col, aggregation in enumerate(aggregations):
            file_normal = f'uncertainty_values.pth'
            file_conflict = f'conflict_values.pth'
            try:
                # normal_uncertainty = torch.load(
                #     file_normal).detach().cpu().numpy()
            ...
            ax = axes[row, col]
            sns.kdeplot(conflict_uncertainty.reshape(-1),
                        color='#ff7f0e', label='Conflict', ax=ax, fill=True)
            sns.kdeplot(normal_uncertainty.reshape(-1),
                        color='#1f77b4', label='Normal', ax=ax, fill=True)
            ax.set_xlim([0, 1])
            ...
            uncertainties = np.concatenate(
                [normal_uncertainty, conflict_uncertainty])
            labels = np.concatenate(
                [np.zeros_like(normal_uncertainty), np.ones_like(conflict_uncertainty)])
            fpr, tpr, _ = roc_curve(labels, uncertainties)
            roc_auc = auc(fpr, tpr)
            ax.text(0.75, 0.9, f'AUC: {roc_auc:.2f}', fontsize=15,
                    ha='center', va='center', transform=ax.transAxes)
            if row == 0:
                ax.set_title(
                    aggregation_labels[aggregation], fontsize=17, weight='bold' if col == 2 else 'normal')
    ...
    for row, dataset in enumerate(datasets):
        ...
        if row == 0:
            y_position += 0.02
        fig.text(0.5, y_position, f'{dataset}', ha='center',
                 va='bottom', fontsize=18, weight='bold')

    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='lower center', ncol=2,
               fontsize=15, bbox_to_anchor=(0.5, -0.05))
...
def plot_average_modality_uncertainties(datasets, output_filename):
...
def generate_latex_table(exp_results_dir):
    df = pd.concat([pd.read_csv(f'{exp_results_dir}/{i}')
                   for i in os.listdir(exp_results_dir) if i.endswith('.csv')])
    df = df[df['aggregation'] != 'weighted_doc']
    grouped_df = df.groupby(['dataset', 'aggregation']).agg(
        {'auc': 'mean', 'auc_std': 'mean'}).reset_index()

    # Create "mean ± std" formatted string for each row
    grouped_df['mean_std'] = grouped_df['auc'].map(
        '{:.2f}'.format) + ' ± ' + grouped_df['auc_std'].map('{:.2f}'.format)

    # Pivot the DataFrame to get datasets as columns and methods as rows
    pivot_formatted_df = grouped_df.pivot(
        index='aggregation', columns='dataset', values='mean_std')
